...
def wasserstein(X,t,p,m,lam=10,its=10,sq=False,backpropT=False):
    ...
    Xc = tf.gather(X,ic)
    Xt = tf.gather(X,it)
    nc = tf.compat.v1.to_float(tf.shape(Xc)[0])
    nt = tf.compat.v1.to_float(tf.shape(Xt)[0])
    ...
    a = tf.concat([p*tf.ones(tf.shape(tf.where(t==m)[:,0:1]))/nt, (1-p)*tf.ones((1,1))], 0)
